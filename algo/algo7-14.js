...
function measureWaterLevels(waterLevels) {
  ...
  for (let i = 0; i < waterLevels.length; i++) {
    for (let j = i + 1; j < waterLevels.length; j++) {
      if (waterLevels[j] < waterLevels[j - 1]) {
        break;
      } else if (waterLevels[j] > waterLevels[j - 1]) {
        trackRise = waterLevels[j] - waterLevels[i];
      }
    }
    if (trackRise > maxRise) {
      maxRise = trackRise;
    }
  }
  return maxRise;
...
function symmetricDifferences(numsA, numsB) {
  ...
  const numsBTable = buildFreqTable(numsB);

  for (const key in numsATable) {
    if (!numsBTable[key]) {
      differenceArray.push(key);
    }
  }
  for (const key in numsBTable) {
    if (!numsATable[key]) {
      differenceArray.push(key);
    }
  }
